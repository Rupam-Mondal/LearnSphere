...
const getCourseDetails = async (req, res) => {
  try {
    ...
    if( courseDetails.students && !courseDetails.students.includes(decoded.id)) {
      return res.status(200).json({ success: true, enrolled:false , course: courseDetails });
    }

    if (courseDetails.students.includes(decoded.id)) {
      res.status(200).json({ success: true, enrolled:true, course: courseDetails });
    }
...
export { getAllCourse, getCourseDetails };
